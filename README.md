...
## 퀴즈 주제 선정 
기본- 대한민국, 미국, 중국 (나라 퀴즈, 각 나라당 5개의 문제, 문제 추가 삭제 가능)
응용- 파이썬 관련 2문제 (프로젝트 개요의 05단계 이후부터 이 주제로 변경함)
...
특징: 키(Key)와 값(Value)으로 이루어진 구조로, 사람이 읽기 쉽고 컴퓨터가 처리하기 빠릅니다.
역할: 퀴즈의 문제(question), 4개의 선택지(options), 정답 번호(answer)를 리스트 형태로 저장합니다.


## 주요 기능
1. 기본 기능 (Basic)
주제별 퀴즈 선택: 대한민국, 중국, 미국 등 국가별 주제를 선택하여 학습할 수 있습니다. (주제별 5문제 구성)
사용자 정의 문제 추가: 기존 문제 외에 사용자가 직접 새로운 문제와 정답을 등록하여 문제 은행을 확장할 수 있습니다.
2. 응용 및 고도화 기능 (Advanced)
콘텐츠 확장: 파이썬 기초 개념 문제를 추가하여 학습 범위를 넓혔습니다.
사용자 식별 시스템: 사용자 이름을 입력받아 개인별 퀴즈 세션을 관리하고 몰입감을 높였습니다.
게임 로직 고도화:
문제 셔플(Shuffle): random 모듈을 활용해 매번 문제 순서를 섞어 중복 학습의 지루함을 방지했습니다.
결과 분석: 총점 계산 및 정답률(%)을 시각화하여 학습 성취도를 즉각 확인하게 했습니다.
프로그램 안정성(Exception Handling):
숫자가 아닌 문자 입력 시 오류 방지
선택지 범위(1~4)를 벗어난 입력값 처리
빈 칸 입력 방지 등 견고한 예외 처리를 구현했습니다.
...
-Step 05. 예외 처리 (Exception Handling): 사용자가 숫자가 아닌 값을 입력하거나, 범위를 벗어난 번호를 입력했을 때 프로그램이 종료되지 않도록 try-except와 조건문으로 방어 코드를 작성했습니다.
### 05단계 실행 결과 스크린샷
...
2. 클래스와 객체 (OOP)
클래스 (Class): 객체를 만들기 위한 설계도(붕어빵 틀)입니다. 비슷한 기능을 하는 변수와 함수를 하나로 묶기 위해 사용
